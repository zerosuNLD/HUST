...
for i in range(len(inputs_test_dataset)):
    inputs_test_dataset[i], targets_test_dataset[i]  = transforms(inputs_test_dataset[i], targets_test_dataset[i]) 



inputs_test_dataset = np.array(inputs_test_dataset)
targets_test_dataset =  np.array(targets_test_dataset)

print(inputs_test_dataset[0].shape)
inputs_test = np.expand_dims(inputs_test_dataset, axis=0)

test_target_predict = model.predict(inputs_test[1])
predicted_img = np.argmax(test_target_predict, axis=3)[0,:,:]

plt.figure(figsize=(12, 8))
plt.subplot(231)
plt.title('Testing Image')
plt.imshow(inputs_test_dataset[1])
plt.subplot(232)
plt.title('Testing Label')
plt.imshow(targets_test_dataset[1])
plt.subplot(233)
